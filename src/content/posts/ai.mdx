...
Here is an image of the current main page that they (aka the AI wrote) compared to a page I wrote using the style system we use (Shadcn/ui): (all information relating to the school has been blocked out as to protect me and the school.)
...
- We need standards other than robots.txt or Cloudflare's tools to stop AI from scraping our websites, such as a new meta tag standard, something like this:
...
For art and video we need to make it opt-in like above, and have the ability to require credit and (possibly) royalties for works used.
